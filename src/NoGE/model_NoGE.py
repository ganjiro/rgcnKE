...
class NoGE_QGNN_QuatE(torch.nn.Module):
    ...
    def forward(self, e1_idx, r_idx, lst_indexes):
        ...
        for _layer in range(self.num_layers):
            X = self.lst_qgnn[_layer](X, self.adj)
        h = X[e1_idx.long()]  # XXX MY LONG FIX
        r = X[((r_idx + self.n_entities).long())]  # XXX MY LONG FIX
        T = X[:self.n_entities]
        if self.variant == "D":
...
class NoGE_GCN_QuatE(torch.nn.Module):
    ...
    def forward(self, e1_idx, r_idx, lst_indexes):
        ...
        h = X[
            e1_idx]  # I.e., simply splitting an embedding into 4 quaternion components, slightly better than using X=np.tile(X, 4) in preliminary experiments. Note that when using X=np.tile(X, 4), QuatE becomes DistMult
        r = X[r_idx + self.n_entities]
        normalized_r = normalization(r)
        hr = vec_vec_wise_multiplication(h, normalized_r)  # following the 1-N scoring strategy
        hr = self.bn1(hr)
