        ...
        if self.encoder.lower() == "gcn":
            ...
            if self.decoder.lower() == "quate":
                model = NoGE_QGNN_QuatE(self.emb_dim, self.hid_dim, adj, len(self.entity_idxs), len(self.relation_idxs),
                                        self.num_layers, self.variant).to(device)
                print("and the customized QuatE decoder...")
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default="FB15k-237", nargs="?", help="codex-s, codex-m, and codex-l") # XXX change deafult (codex-s)
    parser.add_argument("--num_iterations", type=int, default=5, nargs="?", help="Number of iterations.") # XXX prima era 3000
